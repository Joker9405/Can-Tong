...
export default async function handler(req,res){
  ...
  const body=req.body||{}; const entries = Array.isArray(body.entries) ? body.entries : [];
  let ok=0, fail=0, wrote=0;
  for(const raw of entries){
    const z=splitVariants(raw.zhh||''); const c=splitVariants(raw.chs||''); const e=splitVariants(raw.en||'');
    const nonEmpty=[z.length>0,c.length>0,e.length>0].filter(Boolean).length; if(nonEmpty<2){ fail++; continue; }
